...
void solveHanoi(int n, string &source, string &extra, string &target, int &currentMove) {
    ...
    solveHanoi(n - 1, extra, source, target, currentMove); //after the first half of recursion finishes, we have to take care of the next disc's new source which is now the extra peg
}

int main() {
    int n; //number of discs you want this game to have
